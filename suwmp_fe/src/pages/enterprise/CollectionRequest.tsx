...
function CollectionRequest() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await wasteReportService.getWasteReportsByEnterprise(
          2
        );
  ...
  const filteredRequests = fetchedRequests.filter((req) => {
    if (statusFilter !== "all" && req.currentStatus !== statusFilter)
      return false;
    if (
      searchQuery &&
      !req.requestId
  ...
  return (
    ...
    >
      <div>
        <header
          className="fixed top-0 left-0 w-full lg:left-[250px] lg:w-[calc(100%-250px)]
        ...
        >
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Manage and assign waste collection requests
            </p>
          </div>
        ...
        <main className="space-y-6 pt-[120px] px-6">
          ...
          <Card className="overflow-hidden text-foreground shadow-md">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    ...
                    <th className="text-right py-3 pr-6 text-sm font-medium text-muted-foreground">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {filteredRequests.map((req, index) => {
                    const status = statusConfig[
                      req.currentStatus as keyof typeof statusConfig
                    ] || {
                      label: req.currentStatus,
                      color: "bg-gray-100 text-gray-700",
                      icon: Circle,
                    };
                    const priority = priorityConfig[
                      req.priority as keyof typeof priorityConfig
                    ] || {
                      label: "Normal",
                      color: "bg-gray-100 text-gray-700",
                    };
                      ...
                      >
                        <td className="py-4 px-4">
                          <Checkbox
                            ...
                            onCheckedChange={() =>
                              toggleSelect(req.requestId.toString())
                            }
                            className="border-2 border-black/50"
                          />
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <Badge
                            variant="outline"
                            className={`text-xs ${
                              req.wasteTypeName === "Recyclables"
                                ? "bg-blue-50 text-blue-700 border-blue-200"
                                : req.wasteTypeName === "Organic"
                                ? "bg-green-50 text-green-700 border-green-200"
                                : req.wasteTypeName === "E-Waste"
                        ...
                        <td className="py-3 px-4 max-w-[200px]">
                          <div className="text-sm">
                            ...
                            <p className="pt-1 text-muted-foreground truncate">
                              {req.citizenPhone}
                            </p>
                          </div>
                        </td>
            ...
            <div className="flex items-center justify-between p-4 border-t border-border">
              <p className="text-sm text-muted-foreground">
                Showing {filteredRequests.length} of {fetchedRequests.length}{" "}
                requests
              </p>
              <div className="flex items-center gap-2">
